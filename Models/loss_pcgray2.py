...
class PCGray2Loss:
    ...
    def get_loss(self, data, epoch_no):
        #! invertibility loss
        reconLoss_idx = l2_loss(data['pred_ABs'], data['target_ABs']) + 0.5*l2_loss(data['pred_Ls'], data['target_Ls'])
        #! gradient loss
        ref_gray = basic.rgb2gray(data['target_colors'])
        tvar_pred = computeTotalVariation(data['pred_grays'])
        tvar_refe = computeTotalVariation(ref_gray)
        gradLoss_idx = (tvar_pred-tvar_refe).abs()
        #! brightness mapping loss
        '''
        diff_map = (data['pred_grays']-ref_gray).abs()
        ...
        mappingLoss_idx = torch.max(thresMap, (data['pred_grays']-ref_gray).abs()).mean() - 70/127.0
        #! contrast loss
        N,C,H,W = data['pred_grays'].shape
        pred_grayRGB = data['pred_grays'].expand(N,3,H,W)
        contrastLoss_idx = self.vgg_loss(pred_grayRGB*0.5+0.5, data['target_colors']*0.5+0.5)
